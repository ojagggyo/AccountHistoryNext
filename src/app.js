	...
	console.log(`getPriceBitpoint name=${name} markets=${markets}`);
      ...
      return new Promise((resolve, reject) => {
		...
		window['get'+name] = async function(data){
			...
			let price = parseInt((parseFloat(bidPrice)+parseFloat(askPrice))/2);
...
window.showTooltip_post = async (e) => {
    ...
    .then(result => {
        ...
        createTooltip(pageX, pageY, scrollY, result, author);  // 結果を使ってツールチップを作成
    })
    .catch(error => {
...
function createTooltip(pageX, pageY, scrollY, result, author) {
